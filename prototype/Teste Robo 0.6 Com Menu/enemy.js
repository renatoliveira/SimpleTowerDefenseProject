...
function Enemy(speed){
	...
	*/
	...
	{
		if (this.x > canvas.paths.width ||
			this.y > canvas.paths.height)
		{
			//console.log("saiu");
			return true;
		}
	...
	this.updateRobotPosition = function ()
	{
		...
		else if (orientacao == 'right' &&  this.x > 100 + ((this.indiceMatriz % 10)+1)*64) //checa se a posição do Inimigo é maior que o centro do Tile
		{
			this.indiceMatriz += 1;
			flag = 1;
		}
		else if (orientacao == 'left' &&  this.x < 100 + ((this.indiceMatriz % 10)-1)*64)//checa se a posição do Inimigo é menor que o centro do Tile
		{
			this.indiceMatriz -= 1;
			flag = 1;
		}
